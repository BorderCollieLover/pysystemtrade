...
def update_repo_multiple_prices_for_instrument (instrument_code): 
    ...
    multiple_prices_from_db = os.path.join('data', 'futures', 'multiple_from_db') # Generated multiple prices from contract price parquets for 
    ...
    #4.1 While it is generally useful to remove the last line of the generated roll calendar to avoid spurious rolls, it is possible that the last line is an acutal roll
    ...
    #Slightly complicated if the generated multiple prices is completed removed due to stale PRICE_CONTRACT, in which case we need to remove the file on the drive as well to avoid confusion in the next step of splicing.
    #This way we don't have to change the above process_multiple_prices_single_instrument function (from existing PST repository) to deal with the possibility of empty multiple prices generated.
    stale_price_contract_exists = False
    while not generated_multiple_prices.empty: 
        if generated_multiple_prices.iloc[-1].PRICE is not None and not pd.isna(generated_multiple_prices.iloc[-1].PRICE):
            break
        else:
            generated_multiple_prices = generated_multiple_prices.iloc[:-1]
            stale_price_contract_exists = True

    if stale_price_contract_exists:
        multiple_prices_generated_from_parquets = csvFuturesMultiplePricesData(multiple_prices_from_db)

        if generated_multiple_prices.empty:
            print(f"Generated multiple prices for {instrument_code} is empty after removing rows without PRICE, skipping splicing")
            #remove the generated_multiple_prices file 
            #multiple_prices_generated_from_parquets.remove_multiple_prices_file(instrument_code)
            os.remove(os.path.join(multiple_prices_from_db, instrument_code + '.csv'))
            return None
        else:
            #save the updated generated multiple prices after removing the stale price contract rows, which is needed for the next step of splicing with the existing multiple prices
            multiple_prices_generated_from_parquets._add_multiple_prices_without_checking_for_existing_entry(instrument_code, generated_multiple_prices) 
